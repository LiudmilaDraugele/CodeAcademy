...
//reikes padaryti, kad update ir delete funkcijose taskas graziai populaitintusi, 
//tik is to ka employee jau moka ant change, o ne bendru srautu
...
async function populateTaskSelect() {
    ...
    .then(data => {
        ...
        console.log("task lists in competency page populated")}); 
}
...
async function populateLevelSelect() {
        ...
        console.log("level list in competency page populated")
    }
...
async function updateCompetency(e){
    ...
    //baigiau cia - reikia patikriti ar gerrai getina ir updeitina, kad isrinktu reikalinga updaitinti competencija
   ...
   .then(data => data.forEach(element => {
    if (myemployeeid == element.employeeId && mytaskid == element.taskId){
        mycompid = element.id;
        console.log(`atrinkta kompetencija :${mycompid}`);
    }}
   ))
...
async function deleteCompetency(){
   ...
   .then(data => data.forEach(element => {
    if (myemployeeid == element.employeeId && mytaskid == element.taskId){
        mycompid = element.id;
        console.log(`atrinkta istrinimui kompetencija :${mycompid}`);
    }}
   ))
   await fetch(`http://localhost:8886/planning/api/competency/${mycompid}`,{
        method: "DELETE",
